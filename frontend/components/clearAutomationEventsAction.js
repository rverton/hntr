...
export default function ClearAutomationEventsAction({ boxid, mutate }) {

  const clear = (status) => {
    api.post(`/box/${boxid}/_clear?status=${status}`)
      .then(() => {
        mutate()
      })
      .catch(err => {
